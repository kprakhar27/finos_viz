...
export class Client {
  ...
  post(msg, resolve, reject, keep_alive = false) {
    ...
    if (this._worker.initialized.value) {
      ...
      this._worker.messages.push(() => {
        ...
        if ((msg.cmd === "table" || msg.cmd === "view") && !((_this$_features = this._features) !== null && _this$_features !== void 0 && _this$_features.wait_for_response) && resolve) {
          resolve();
        }
      });
    }
  ...
  _handle(e) {
    if (!this._worker.initialized.value) {
      ...
      this._worker.messages = []; // If the `data` attribute of the init message is set, then
      // set the `features` dictionary with the flags from the server.

      if ((_e$data = e.data) !== null && _e$data !== void 0 && _e$data.data) {
        this._features = {};
