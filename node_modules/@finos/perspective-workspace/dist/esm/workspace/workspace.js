...
class ObservableMap extends Map {
  set(name, item) {
    var _this$_set_listener;

    (_this$_set_listener = this._set_listener) === null || _this$_set_listener === void 0 ? void 0 : _this$_set_listener.call(this, name, item);
    super.set(name, item);
  }
  ...
  delete(name) {
    var _this$_delete_listene;

    const result = (_this$_delete_listene = this._delete_listener) === null || _this$_delete_listene === void 0 ? void 0 : _this$_delete_listene.call(this, name);
...
export class PerspectiveWorkspace extends DiscreteSplitPanel {
  constructor(element, options = {}) {
    ...
    _defineProperty(this, "onPerspectiveSelect", event => {
      const config = event.target.save(); // perspective-select is already handled for hypergrid
    ...
    _defineProperty(this, "workspaceUpdated", () => {
      if (!this._save) {
    ...
    this.masterPanel = new DiscreteSplitPanel({
  ...
  *_capture_viewers() {
  ...
  _restore_callback(viewers, starting_viewers, starting_widgets, master, widgetName) {
    ...
    if (master || this.mode === MODE.LINKED) {
      ...
      widget.viewer.addEventListener("perspective-click", this.onPerspectiveSelect); // TODO remove event listener
  ...
  toggleMasterDetail(widget) {
    ...
    this.element.dispatchEvent(new CustomEvent("workspace-toggle-global-filter", {
      detail: {
        widget,
  ...
  _linkWidget(widget) {
    ...
    if (this._linkedViewers.indexOf(widget.viewer) === -1) {
      this._linkedViewers.push(widget.viewer); // if this is the first linked viewer, make viewers with
  ...
  toggleLink(widget) {
    ...
    if (widget.linked) {
      ...
      if (this._linkedViewers.length === 0) {
        this.getAllWidgets().forEach(widget => widget.viewer.restore({
          selectable: false
        }));
      }
    }
  ...
  showContextMenu(widget, event) {
    ...
    menu.aboutToClose.connect(() => {
      ...
      tabbar === null || tabbar === void 0 ? void 0 : (_tabbar$node = tabbar.node) === null || _tabbar$node === void 0 ? void 0 : _tabbar$node.classList.remove("context-focus");
    });
  ...
  _addContextMenuItem(item) {
    ...
    this.commands.addCommand(item.id, {
      execute: args => item.execute({
        widget: args.widget
      }),
      label: item.label,
      isVisible: args => item.isVisible({
        widget: args.widget
      }),
      mnemonic: 0
    });
  ...
  addViewer(config) {
    const widget = this._createWidgetAndNode({
      config
    });
  ...
  _fireUpdateEvent() {
    ...
    if (layout) {
      ...
      this.element.dispatchEvent(new CustomEvent("workspace-layout-update", {
        detail: {
          tables,
          layout
        }
