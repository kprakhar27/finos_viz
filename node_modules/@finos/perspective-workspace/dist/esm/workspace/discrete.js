...
function cloneMouseEvent(x, y, event) {
  ...
  evt.initMouseEvent(event.type, true, event.cancelable, event.view, event.detail, event.screenX, event.screenY, x, y, event.ctrlKey, event.altKey, event.shiftKey, event.metaKey, event.button, event.relatedTarget);
  return evt;
}
...
class DiscreteContext {
  ...
  calculate_move({
    ...
    if (this._check_prev(clientX, clientY)) {
      return {
        x: clientX + this._rect.left,
        y: clientY + this._rect.top
      };
    }
  }
