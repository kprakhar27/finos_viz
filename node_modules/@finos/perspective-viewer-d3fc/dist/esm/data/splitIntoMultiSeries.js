 ...
 *
 */
export function splitIntoMultiSeries(settings, data, {
  stack = false,
  excludeEmpty = false
} = {}) {
...
function splitByValuesIntoMultiSeries(settings, data, {
  stack = false,
  excludeEmpty = false
}) {
  ...
  data.forEach(col => {
    ...
    const baseValues = {};
    const split = {}; // Keys are of the form "split1|split2|aggregate"

    Object.keys(col).filter(key => key !== "__ROW_PATH__").filter(key => !excludeEmpty || col[key] != null && col[key] != undefined).forEach(key => {
      ...
      splitValues.__KEY__ = splitName; // Assign the values for this split/aggregate
...
//# sourceMappingURL=splitIntoMultiSeries.js.map