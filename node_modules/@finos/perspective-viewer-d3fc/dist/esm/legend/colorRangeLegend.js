...
export function colorRangeLegend() {
  ...
  function legend(container) {
    const legendSelection = getOrCreateElement(container, "div.legend-container", () => container.append("div").attr("class", "legend-container legend-color").style("z-index", "2"));
    const {
      width,
      height
    } = legendSelection.node().getBoundingClientRect();
    const xScale = d3.scaleBand().domain([0, 1]).range([0, width]);
    const domain = scale.copy().nice().domain();
    const paddedDomain = fc.extentLinear().pad([0.1, 0.1]).padUnit("percent")(domain);
    ...
    const svgBar = fc.autoBandwidth(fc.seriesSvgBar()).xScale(xScale).yScale(yScale).crossValue(0).baseValue((_, i) => expandedDomain[Math.max(0, i - 1)]).mainValue(d => d).decorate(selection => {
    ...
    const axisLabel = fc.axisRight(yScale).tickValues(tickValues).tickSizeOuter(0).tickFormat(d => valueformatter(d));
...
//# sourceMappingURL=colorRangeLegend.js.map