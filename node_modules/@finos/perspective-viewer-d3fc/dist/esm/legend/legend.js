...
const scrollColorLegend = settings => scrollableLegend(d3Legend.legendColor().shape("circle").shapeRadius(6), settings);

const scrollSymbolLegend = settings => scrollableLegend(d3Legend.legendSymbol().shapePadding(1).labelOffset(3), settings);
...
function legendComponent(scrollLegendComponent, scaleModifier) {
  ...
  function legend(container) {
    if (scale && scale.range().length > 1) {
      ...
      scrollLegend.scale(scale).orient("vertical").on("cellclick", function (d) {
      ...
      scrollLegend.decorate(selection => {
        ...
        const cells = selection.select("g.legendCells").attr("transform", "translate(20,20)").selectAll("g.cell");
        cells.classed("hidden", isHidden);
        cells.append("title").html(d => d);

        if (color) {
          cells.select("circle, path").style("fill", d => isHidden(d) ? null : color(d)).style("stroke", d => isHidden(d) ? null : withoutOpacity(color(d)));
        }
      }); // render the legend

      legendSelection.attr("class", "legend-container").attr("borderbox-on-hover", true).style("z-index", "2").call(scrollLegend);
    }
  }
