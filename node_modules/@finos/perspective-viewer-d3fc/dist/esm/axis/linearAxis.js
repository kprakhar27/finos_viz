...
export const domain = () => {
  ...
  const getMinimumGap = data => {
    const gaps = valueNames.map(valueName => data.map(d => d[valueName]).sort((a, b) => a - b).filter((d, i, a) => i === 0 || d !== a[i - 1]).reduce((acc, d, i, src) => i === 0 || acc <= d - src[i - 1] ? acc : Math.abs(d - src[i - 1])));
