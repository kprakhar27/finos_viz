...
export const axisSplitter = (settings, sourceData, splitFn = dataSplitFunction) => {
  let color;
  let data;
  let altData; // splitMainValues is an array of main-value names to put into the alt-axis
  ...
  const haveSplit = settings["mainValues"].some(m => altValue(m.name)); // Split the data into main and alt displays

  data = haveSplit ? splitFn(sourceData, key => !altValue(key)) : sourceData;
  altData = haveSplit ? splitFn(sourceData, altValue) : null; // Renderer to show the special controls for moving between axes
  ...
  const splitter = selection => {
    ...
    selection.select(".y2-label-container>.y-label").call(labeller().labels(altLabels).alt(true));
  };
...
export const dataBlankFunction = (sourceData, isIncludedFn) => {
  return sourceData.map(series => {
    if (!isIncludedFn(series.key)) {
      // Blank this data
      return series.map(v => Object.assign({}, v, {
        mainValue: null
      }));
    }
...
//# sourceMappingURL=axisSplitter.js.map