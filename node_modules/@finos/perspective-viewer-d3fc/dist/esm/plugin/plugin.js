...
export function register(...plugins) {
  ...
  charts.forEach(chart => {
    if (plugins.has(chart.plugin.type)) {
      registerPlugin(chart.plugin.type, { ...DEFAULT_PLUGIN_SETTINGS,
        ...chart.plugin,
...
function drawChart(chart) {
  return async function (el, view, task, end_col, end_row) {
    ...
    const filtered = row_pivots.length > 0 ? json.reduce((acc, col) => {
    ...
    const dataMap = (col, i) => !row_pivots.length ? { ...col,
      __ROW_PATH__: [i]
    } : col;
