...
export const clickHandler = (data, g, parent, parentTitle, path, label, radius, split, settings) => (p, skipTransition) => {
  ...
  }).attr("fill-opacity", d => arcVisible(d.target) ? 1 : 0).attr("user-select", d => arcVisible(d.target) ? "initial" : "none").attr("pointer-events", d => arcVisible(d.target) ? "initial" : "none").attrTween("d", d => () => drawArc(radius)(d.current));
  label.filter(function (d) {
    return +this.getAttribute("fill-opacity") || labelVisible(d.target);
  }).transition(t).attr("fill-opacity", d => +labelVisible(d.target)).attrTween("transform", d => () => labelTransform(d.current, radius));
};
//# sourceMappingURL=sunburstClick.js.map