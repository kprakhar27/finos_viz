...
export function sunburstSeries() {
  ...
  const _sunburstSeries = sunburstElement => {
    ...
    const segmentEnter = segment.enter().append("g").attr("class", "segment");
    segmentEnter.append("path");
    segmentEnter.append("text").attr("class", "segment").attr("dy", "0.35em");
    const segmentMerge = segmentEnter.merge(segment);
    const path = segmentMerge.select("path").attr("fill-opacity", d => arcVisible(d.current) ? 1 : 0).attr("user-select", d => arcVisible(d.current) ? "initial" : "none").attr("pointer-events", d => arcVisible(d.current) ? "initial" : "none").attr("d", d => drawArc(radius)(d.current));
    color && path.style("fill", d => color(d.data.color));
    const label = segmentMerge.select("text").attr("fill-opacity", d => +labelVisible(d.current)).attr("transform", d => labelTransform(d.current, radius)).text(d => d.label).each(function (d) {
      cropLabel.call(this, d, radius);
    });
    const parentTitle = sunburstElement.select("text.parent");
    const parent = sunburstElement.select("circle").attr("r", radius).datum(data);
    ...
    path.filter(d => d.children).style("cursor", "pointer").on("click", d => onClick(d, false));
  };
