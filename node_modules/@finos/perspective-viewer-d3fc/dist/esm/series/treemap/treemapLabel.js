...
export const toggleLabels = (nodes, treemapLevel, crossValues) => {
  nodes.selectAll("text").style("font-size", null).attr("class", d => textLevelHelper(d, treemapLevel, crossValues));
...
export const restoreLabels = nodes => {
  nodes.each((d, i, nodes) => {
    ...
    label.attr("dx", d.target.textAttributes.dx).attr("dy", d.target.textAttributes.dy).attr("class", d.target.textAttributes.class).style("font-size", d.target.textAttributes["font-size"]);
  });
};
export const preventTextCollisions = nodes => {
  ...
  const topNodes = [];
  nodes.selectAll("text").filter((_, i, nodes) => select(nodes[i]).attr("class") === textVisability.high).each((_, i, nodes) => topNodes.push(nodes[i]));
  nodes.selectAll("text").filter((_, i, nodes) => select(nodes[i]).attr("class") === textVisability.low).each((_, i, nodes) => {
    const lowerNode = nodes[i];
    topNodes.filter(topNode => isElementOverlapping("x", rect(topNode), rect(lowerNode)) && isElementOverlapping("y", rect(topNode), rect(lowerNode), textCollisionFuzzFactorPx)).forEach(() => select(lowerNode).attr("dy", Number(select(lowerNode).attr("dy")) + textAdjustPx));
  });
};
...
export const selectVisibleNodes = nodes => nodes.filter((_, i, nodes) => select(nodes[i]).selectAll("text").attr("class") !== textVisability.zero);
...
//# sourceMappingURL=treemapLabel.js.map