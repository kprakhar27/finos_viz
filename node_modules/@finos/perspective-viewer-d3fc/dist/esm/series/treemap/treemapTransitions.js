...
function executeTransition(d, rects, nodesMerge, labels, settings, treemapDiv, treemapSvg, rootNode, treemapLevel, crossValues, parentCtrls, duration = 500, recordLabelMap = true) {
  ...
  const t = treemapSvg.transition("main transition").duration(duration).ease(d3.easeCubicOut);
  nodesMerge.each(d => d.target = d.mapLevel[treemapLevel]);
  if (!labelMapExists(d)) preventUserInteraction(nodesMerge, parentCtrls); // hide hidden svgs

  nodesMerge.transition(t).tween("data", d => {
  ...
  if (parent) {
    parentCtrls.hide(false).text(d.label).onClick(() => changeLevel(parent, rects, nodesMerge, labels, settings, treemapDiv, treemapSvg, rootNode, parentCtrls, duration))();
  } else {
...
async function fadeTextTransition(labels, treemapSvg, duration = 400) {
  const t = treemapSvg.transition("text fade transition").duration(duration).ease(d3.easeCubicOut);
  await labels.transition(t).filter(d => d.target.visible).tween("data", (d, i, labels) => {
    const label = labels[i];
    const interpolation = d3.interpolate(lockedOpacity(d), targetOpacity(label));
    return t => d.current.opacity = interpolation(t);
  }).styleTween("opacity", d => () => d.current.opacity).end().catch(ex => console.error("Exception in text fade transition", ex)).then(() => labels.each((_, i, labels) => unlockTextOpacity(labels[i])));
}
...
//# sourceMappingURL=treemapTransitions.js.map