...
export function treemapSeries() {
  ...
  const _treemapSeries = treemapSvg => {
    ...
    color && rects.style("fill", d => {
      if (d.data.color) {
        return color(d.data.color);
      }
    });
    const labels = nodesMerge.filter(d => d.value !== 0).select("text").attr("x", d => d.x0 + calcWidth(d) / 2).attr("y", d => d.y0 + calcHeight(d) / 2).text(d => d.label);
